import "server-only";
import { pool } from "../db";
import type { DbSection as Section } from "../types";

const DEFAULT_SECTIONS: Record<string, Array<{ type: string; title: string; content: Record<string, unknown>; sort_order: number }>> = {
  _base: [
    {
      type: "hero",
      title: "Hero",
...
export async function updateSection(sectionId: number, updates: { title?: string; content?: Record<string, unknown>; visible?: boolean }): Promise<Section> {
  const fields: string[] = [];
